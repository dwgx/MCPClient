...
extends Library {
    ...
    // LWJGL3 OpenAL device/context handles (LWJGL2's AL.create/destroy are gone).
    private static long alDevice = 0L;
    private static long alContext = 0L;

    // LWJGL3 replacement for LWJGL2 AL.create(): open device, create+make-current
    // context, and build AL capabilities. Throws LWJGLException on failure to keep
    // the surrounding paulscode error handling intact.
    private static void lwjgl3CreateAL() throws LWJGLException {
        alDevice = ALC10.alcOpenDevice((java.nio.ByteBuffer) null);
        if (alDevice == 0L) {
            throw new LWJGLException("Failed to open the default OpenAL device");
        }
        ALCCapabilities alcCaps = ALC.createCapabilities(alDevice);
        alContext = ALC10.alcCreateContext(alDevice, (java.nio.IntBuffer) null);
        if (alContext == 0L) {
            ALC10.alcCloseDevice(alDevice);
            alDevice = 0L;
            throw new LWJGLException("Failed to create the OpenAL context");
        }
        ALC10.alcMakeContextCurrent(alContext);
        AL.createCapabilities(alcCaps);
    }

    // LWJGL3 replacement for LWJGL2 AL.destroy().
    private static void lwjgl3DestroyAL() {
        if (alContext != 0L) {
    ...
    private static boolean lwjgl3IsALCreated() {
        return alContext != 0L;
    }
    ...
    public void init() throws SoundSystemException {
        ...
        try {
            ...
            this.printStackTrace((java.lang.Exception)((Object)e));
