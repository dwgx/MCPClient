...
public class OpenGlHelper
{
    ...
    public static void initializeTextures()
    {
        ...
        try
        {
            ...
            cpu = String.format("%dx %s", new Object[] {Integer.valueOf(processor.getLogicalProcessorCount()), processor.getProcessorIdentifier().getName()}).replaceAll("\\s+", " ");
        }
    ...
    public static void glShaderSource(int shaderIn, ByteBuffer string)
    {
        if (arbShaders)
        {
            ARBShaderObjects.glShaderSourceARB(shaderIn, MemoryUtil.memUTF8(string));
        }
        else
        {
            GL20.glShaderSource(shaderIn, MemoryUtil.memUTF8(string));
        }
    }
