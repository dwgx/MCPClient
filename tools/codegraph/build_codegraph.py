#!/usr/bin/env python3
"""Build a call graph of every compiled module from javap output.
...
# Every reactor module that produces classes. The kernel (`core`) is the point
# of the project, so leaving it out — as the client-and-shim-only original did —
# made the graph useless for the work that actually happens here. Kernel first so
# a partial build still indexes what matters most.
CLASS_ROOTS = [
    "core/target/classes",
    "board/target/classes",
    "dwm/target/classes",
    "client/target/classes",
    "lwjgl2-shim/target/classes",
    "pg/pg-api/target/classes",
    "pg/pg-engine/target/classes",
    "pg/pg-maven-plugin/target/classes",
]
BATCH = 60
