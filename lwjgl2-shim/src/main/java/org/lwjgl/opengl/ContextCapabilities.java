...
/**
 * LWJGL2-compatibility capability holder. LWJGL3 removed this class in favour of
 * {@link org.lwjgl.opengl.GLCapabilities}. Minecraft 1.8.9 reads a fixed set of
 * public boolean fields off the object returned by
 * {@code GLContext.getCapabilities()}, so we expose exactly that set here and
 * populate it by reflectively copying the same-named boolean fields from the
 * real LWJGL3 {@link GLCapabilities}.
 *
 * Any field that does not exist on the running LWJGL3 build (legacy fixed-function
 * extensions, ARB_compatibility, etc.) is simply left at its default of false.
 */
public class ContextCapabilities
{
    // --- OpenGL version flags referenced by MC (OpenGlHelper / GuiMainMenu) ---
    ...
    // --- ARB extensions referenced by MC (Minecraft snooper + OpenGlHelper) ---
    ...
    // --- EXT extensions referenced by MC (Minecraft snooper + GuiStreamUnavailable) ---
    ...
    // --- NV extensions referenced by MC (EntityRenderer) ---
    public boolean GL_NV_fog_distance;

    /**
     * Builds a capability set from the LWJGL3 capabilities currently bound to
     * this thread. Requires {@code GL.createCapabilities()} to have already run
     * for the current context. If capabilities are unavailable, every field is
     * left at false.
     */
    public ContextCapabilities()
    {
        GLCapabilities source = null;

        try
        {
            source = GL.getCapabilities();
        }
    ...
    /**
     * Reflectively copies every public boolean field on this object from the
     * identically-named public boolean field on the supplied LWJGL3
     * {@link GLCapabilities}. Fields with no counterpart remain false.
     */
    private void copyFrom(GLCapabilities source)
    {
        ...
        for (i = 0; i < fields.length; ++i)
        {
            ...
            try
            {
                Field origin = GLCapabilities.class.getField(target.getName());

                if (origin.getType() == Boolean.TYPE)
                {
                    target.setBoolean(this, origin.getBoolean(source));
                }
            }
            catch (NoSuchFieldException nosuchfieldexception)
            {
                // Extension absent from this LWJGL3 build; leave field false.
            }
            catch (IllegalAccessException illegalaccessexception)
            {
                // Should not happen for public fields; leave field false.
            }
        }
    }
}