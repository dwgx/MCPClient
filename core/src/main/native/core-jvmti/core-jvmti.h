 ...
 * the Java class net.marcloud.mcp.core.debug.DebuggerBridge via RegisterNatives
 * (in JNI_OnLoad) — NOT via System.loadLibrary alone, because the module is
 * loaded by -agentpath.
 *
 * The exported native functions match DebuggerBridge's private static native
 * declarations EXACTLY (names + JNI signatures). Events call back into
 * DebuggerBridge.onDebugEvent(int kind, Thread, String location, long numeric).
 *
...
/* Event kind ints passed to DebuggerBridge.onDebugEvent. */
