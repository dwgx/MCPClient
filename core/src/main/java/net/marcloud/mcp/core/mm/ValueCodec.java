    ...
    static Object coerce(Class<?> target, Object json, RootResolver roots) {
        if (json == null) {
            if (target.isPrimitive()) {
                throw new MmAccessException("cannot coerce null to primitive " + target.getName());
            }
            return null;
        }
        ...
        if (target == double.class || target == Double.class) {
            ...
            throw new MmAccessException("cannot coerce " + json.getClass().getName() + " to double");
        }
        if (target == float.class || target == Float.class) {
            if (json instanceof Number n) {
                return n.floatValue();
            }
            throw new MmAccessException("cannot coerce " + json.getClass().getName() + " to float");
        }
        ...
        if (target == boolean.class || target == Boolean.class) {
            ...
            throw new MmAccessException("cannot coerce " + json.getClass().getName() + " to boolean");
        }
        if (target == char.class || target == Character.class) {
            if (json instanceof String s && s.length() == 1) {
                return s.charAt(0);
            }
            throw new MmAccessException("cannot coerce " + json + " to char (need 1-char String)");
        }
        ...
        if (json instanceof java.util.Map<?, ?> map) {
            if (map.containsKey("$path")) {
                if (roots == null) {
                    throw new MmAccessException("$path resolution requires RootResolver");
                }
            ...
            if (map.containsKey("$class") && map.containsKey("$field")) {
                ...
                throw new MmAccessException("$class+$field resolution not yet implemented");
            }
        }
        ...
        throw new MmAccessException("cannot coerce " + json.getClass().getName() + " to " + target.getName());
    }
        ...
        return switch (name) {
            ...
            default -> {
                try {
                    ...
                    throw new MmAccessException("unknown type: " + name, e);
                }
            }
        };
    }
