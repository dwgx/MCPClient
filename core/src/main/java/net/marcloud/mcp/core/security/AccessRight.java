...
public enum AccessRight {
    ...
    public static int mask(AccessRight... rs) {
        int m = 0;
        for (AccessRight r : rs) {
            m |= r.bit;
        }
        return m;
    }
    ...
    public static int mask(Set<AccessRight> rs) {
        int m = 0;
        for (AccessRight r : rs) {
            m |= r.bit;
        }
    ...
    public static EnumSet<AccessRight> decode(int mask) {
        EnumSet<AccessRight> s = EnumSet.noneOf(AccessRight.class);
        for (AccessRight r : values()) {
            if (r.in(mask)) {
                s.add(r);
            }
        }
