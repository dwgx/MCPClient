 ...
 * ObManager} (package-private ctor). Two invariants give the TOCTOU
 * guarantee:
 *
 * <ol>
 ...
 * </ol>
 *
 * <p>{@link AutoCloseable}: closing deregisters from the {@link ObManager}
 * and runs resource cleanup exactly once.
 */
public final class ObHandle implements AutoCloseable {

    private final long id;
    private final String owner;
    private final ObRef ref;
    ...
    private final Consumer<ObHandle> onClose;   // ObManager dereg + resource cleanup

    ObHandle(long id, String owner, ObRef ref, int mask, Object target,
                 long now, Consumer<ObHandle> onClose) {
        this.id = id;
        this.owner = owner;
        this.ref = ref;
    ...
    boolean permits(int need) {
        return !closed.get() && ObAccessMask.subset(frozenMask, need);
    }
