 ...
 *       {@link net.marcloud.mcp.core.se.SeProtectedObjects} (the
 ...
 */
public final class FltDynamicManager implements HookSource, AutoCloseable {
    ...
    /**
     * Construct a FltDynamicManager. Wires {@link HookBridge} to the given bus
     ...
     */
    public FltDynamicManager(Instrumentation inst, EventBus bus) {
        this.inst = inst;
        this.bus = bus;
     ...
     * {@link net.marcloud.mcp.core.ke.event.events.HookFiredEvent} to the bus.
     *
    ...
    public synchronized String install(String targetClassName, String methodName,
        ...
        // Delegates to the canonical SeProtectedObjects set (one source of truth,
        // shared with the redefine guard). Runs before the inst==null gate so it
        // is verifiable in tests without a live agent.
        if (net.marcloud.mcp.core.se.SeProtectedObjects.isProtected(targetClassName)) {
    ...
    public void close() {
        ...
        if (failed > 0) {
            System.err.println("[FltDynamicManager] " + failed
