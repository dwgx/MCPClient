...
public final class PermissionTools {
    ...
    public PermissionTools(SeReferenceMonitor engine, IoManager registry) {
        this.engine = engine;
    ...
    private SyncToolSpecification dropPrivilege() {
        ...
        return new SyncToolSpecification(tool, (exchange, request) -> {
            ...
            // local cache). SeRemoteMonitor fails closed by throwing; render that
            // as an honest tool error, never a phantom "clearance is now Rn".
            final Ring now;
            try {
                now = engine.dropTo(target);
            } catch (net.marcloud.mcp.core.se.SeRemoteMonitor.AuthorityUnreachableException e) {
