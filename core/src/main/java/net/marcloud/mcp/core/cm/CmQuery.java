 ...
 * fallback when the agent is absent), and consults {@link SeProtectedObjects} to
...
public final class CmQuery {
    ...
    public CmQuery(ClassLoader gameLoader, List<HookSource> hookSources) {
    ...
    public ClassDetail describeClass(String fqn) {
        ...
        try {
            ...
            boolean prot = SeProtectedObjects.isProtected(fqn);
    ...
    private ClassInfo toInfo(Class<?> c) {
        ...
        return new ClassInfo(c.getName(), loader, module, true, jvmModifiable(c),
                SeProtectedObjects.isProtected(c.getName()));
    }
    ...
    private MethodInfo toMethodInfo(Method m) {
        return new MethodInfo(m.getDeclaringClass().getName(), m.getName(),
                ...
                Arrays.stream(m.getParameterTypes()).map(CmQuery::typeName).toList(),
    ...
    private MethodInfo toConstructorInfo(Constructor<?> ctor) {
        return new MethodInfo(ctor.getDeclaringClass().getName(), "<init>", "void",
                Arrays.stream(ctor.getParameterTypes()).map(CmQuery::typeName).toList(),
