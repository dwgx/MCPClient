 ...
 * Unsafe fallback for final writes. Refuses {@link SeProtectedObjects} on mutating
...
public final class MmAccess {
    ...
    public MmAccess(GameAccess game, AccessGate gate,
     ...
     *
     * @throws MmAccessException if field not found
     */
    public Object getField(Object t, String name) {
        gate.require(CapabilitySid.CAP_MEMORY_READ);
        if (t == null) {
            throw new MmAccessException("target is null");
        }
        ...
        try {
            ...
            throw new MmAccessException("getField(" + t.getClass().getName() + "." + name + ") failed: " + e.getMessage(), e);
        }
    }
    ...
    public Object getStaticField(Class<?> owner, String name) {
        ...
        try {
            ...
            throw new MmAccessException("getStaticField(" + owner.getName() + "." + name + ") failed: " + e.getMessage(), e);
        }
    }
    ...
    public void setField(Object t, String name, Object value, RootResolver roots) {
        ...
        if (Modifier.isFinal(f.getModifiers())) {
            ...
            try {
                ...
                throw new MmAccessException("setField VarHandle failed: " + e.getMessage(), e);
            }
        }
    }
    ...
    public void openModule(Module target, String pkg) {
        ...
        if (inst == null) {
            throw new MmAccessException("Instrumentation unavailable");
        }
        ...
        try {
            ...
            throw new MmAccessException("openModule failed: " + e.getMessage(), e);
        }
    }
    ...
    private MethodHandles.Lookup lookupFor(Class<?> c) {
        return lookupCache.computeIfAbsent(c, k -> {
            try {
                ...
                throw new MmAccessException("privateLookupIn(" + k.getName() + ") failed: " + e.getMessage(), e);
            }
        });
    }
    ...
    private VarHandle findVarHandle(Class<?> owner, String name, boolean isStatic) {
        ...
        return vhCache.computeIfAbsent(key, k -> {
            ...
            try {
                ...
                throw new MmAccessException("findVarHandle(" + owner.getName() + "." + name + ") failed: " + e.getMessage(), e);
            }
        });
    }
    ...
    private Field resolveField(Class<?> start, String name) {
        ...
        throw new MmAccessException("field " + name + " not found in " + start.getName() + " or superclasses");
    }
    ...
    private Method resolveMethod(Class<?> start, String name, Class<?>[] paramTypes) {
        ...
        throw new MmAccessException("method " + name + " not found in " + start.getName());
    }

    private void guardProtected(Class<?> c) {
        if (SeProtectedObjects.isProtected(c.getName())) {
            throw new MmAccessException("refusing to mutate protected class " + c.getName());
        }
    }
