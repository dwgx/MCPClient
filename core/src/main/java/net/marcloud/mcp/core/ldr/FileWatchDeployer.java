 ...
 * them through the {@link LdrEngine} as they are created or modified. This
 ...
 *
 * <p>The {@code onDeploy} callback receives each {@link LdrEngine.LoadOutcome}
 * so callers can surface results to the AI / logs.
 */
public final class FileWatchDeployer {

    private final Path root;
    private final LdrEngine engine;
    private final Consumer<LdrEngine.LoadOutcome> onDeploy;
    private volatile Thread thread;
    private volatile boolean running;

    public FileWatchDeployer(Path root, LdrEngine engine,
                             Consumer<LdrEngine.LoadOutcome> onDeploy) {
        this.root = root;
        this.engine = engine;
    ...
    private void watchLoop() {
        try (WatchService ws = FileSystems.getDefault().newWatchService()) {
            ...
            onDeploy.accept(new LdrEngine.LoadOutcome(false,
    ...
    private void deploy(Path file) {
        try {
            ...
            LdrEngine.LoadOutcome outcome = engine.loadNew(className, source);
