...
/**
 * The subject of an access decision — the security principal a {@link IoRequestPacket}
 * runs as. Bundles the five in-JVM security dimensions the {@link SeReferenceMonitor}
 * evaluates:
 * <ul>
 ...
 * at a different ring so the live clearance from {@link SeClearancePolicy} can be
 ...
 */
public record SeToken(String tokenId,
                              Ring clearance,
     ...
     */
    public static SeToken wideOpen() {
        return new SeToken("dev", Ring.R_MINUS_1, IntegrityLevel.SYSTEM,
                PrivilegeToken.wideOpen(), null);
    }

    /** A copy at a different ring clearance (fold in the live drop/restore state). */
    public SeToken withClearance(Ring newClearance) {
        if (newClearance == clearance) {
            return this;
        }
        return new SeToken(tokenId, newClearance, integrity, privileges, capabilities);
    }
     ...
     */
    public SeToken withCapabilities(Set<CapabilitySid> newCapabilities) {
        return new SeToken(tokenId, clearance, integrity, privileges, newCapabilities);
    }
