...
/**
 * The result of a {@link SeReferenceMonitor} evaluation: allow/deny plus, on deny, a
 ...
 */
public record SeAccessCheck(boolean allow, String layer, String reason) {

    private static final SeAccessCheck ALLOWED = new SeAccessCheck(true, "L1-L7", "");

    /** A uniform allow (every layer passed). */
    public static SeAccessCheck allowed() {
        return ALLOWED;
    }

    /** A deny attributed to {@code layer} with a human {@code reason}. */
    public static SeAccessCheck deny(String layer, String reason) {
        return new SeAccessCheck(false, layer, reason);
    }
