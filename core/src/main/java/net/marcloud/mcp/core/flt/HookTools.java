 ...
 * dynamic ones via the {@link HookSource} SPI ({@link FltDynamicManager}
 * implements it) — so HookTools does not register its own list tool.
 *
 * <p>The tools wrap {@link FltDynamicManager} and translate its
 * exceptions/results into MCP {@link CallToolResult} ok/err responses, matching
 * the idiom in {@link net.marcloud.mcp.core.io.MetaTools}.
 */
public final class HookTools {

    private final FltDynamicManager mgr;
    private final AccessGate gate;

    /**
     * @param mgr  the FltDynamicManager to wrap
     * @param gate the L4/L5 defense-in-depth gate (require CAP_CLASS_RETRANSFORM)
     */
    public HookTools(FltDynamicManager mgr, AccessGate gate) {
        this.mgr = mgr;
        this.gate = gate;
    }
     ...
     *
     * @param registry the IoManager to register into
     */
    ...
    private SyncToolSpecification installHook() {
        Tool tool = Tool.builder()
                ...
                .description("Install a runtime bytecode hook on a method (any loaded class, any "
                        ...
                        + "CAP_CLASS_RETRANSFORM. Protected kernel classes (SeClearancePolicy, "
