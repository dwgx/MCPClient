 ...
 * SyncToolSpecification}, and hand it to the {@link IoManager}.
 *
...
public final class DynamicToolFactory {
    ...
    private final LdrEngine hotLoad;

    public DynamicToolFactory(LdrEngine hotLoad) {
    ...
    public BuildResult build(String toolName, String className, String description, String source) {
        LdrEngine.LoadOutcome outcome = hotLoad.loadNew(className, source);
        if (!outcome.success()) {
        ...
        SyncToolSpecification spec = new SyncToolSpecification(tool, (exchange, request) -> {
            ...
            try {
                ...
                // Let a genuine tool fault propagate so IoSupervisor's boundary
