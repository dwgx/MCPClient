...
0. **任务是内核/游戏行动能力?先读 `agency/handoff-2026-08-09.md`**(最新交接 ——
   **`eval_java` 的实际边界量出来了:门是好的,门里面没有沙箱**;§4 那条 owner 待决
   ...
   §0 那条活状态告警**只剩一半成立**:客户端已自行退出并 flush(2026-08-05 18:35),
   **但 `test_run/saves/smoke_world` 仍然是被改过的状态**(石台/深坑/落地平台/背包);
   §4 记着上一轮 Bash 静默的真因是
   ...
   **D 也已定(2026-08-05,owner):代码持环 20Hz,模型下目标** —— 而定它的方式要读清楚,
   不是权衡之后选了这一侧,是原来记的那个代价基于一个错误前提(`handoff-2026-08-07.md` §8.0①)。
   **于是 planner / 邻居生成器与 craft 都不再被挡住。**
   任务是 UI?跳过它们,走下面四份。
...
| 文档 | 内容 | 状态 |
|---|---|---|
| `agency/handoff-2026-08-10.md` | **最新交接** —— **59 条候选 59 条全部 SURVIVED**(11 个子系统全 100% 空转,累计 70/70);**唯一安全类那条已修**(§9.1:守卫是源码文本断言 —— **第四种判据形状**;而它那句「从外部驱动不了」是假的);`mutate.py` 两处缺口已修;§9.2 五处漂移;telly 计划的拆分 | **接手先读这份** |
...
| `agency/handoff-2026-08-08.md` | 上一份交接,**测量部分全部有效** —— 搭桥的全部真机测量(**客户端无速率门、服务端 reach 是 8 格不是 4.5、探针打不中一个 tick**)、MANEUVER 三通道锁相分析、§4 一轮偏移的记录。**§0 活状态告警只剩存档那半**(客户端已退出) | 当前 |
...
| `agency/mutation-candidates-2026-08-05.json` | 12 个子系统的 72 个变异候选,**已全部跑完**(每条带 `secondPassVerdict`)。**71 跑过、1 条锚点已失效**;累计 **70/70 SURVIVED**。逐条锚点与替换文本仍在里面,是修复战役的工单 | 当前,**已变现为 59 条待修** |
...
| `agency/command-to-action.md` | 行动/感知层现状、差距、关键路径(§4 带进度横幅)、六个 Fork(**B/C/D 已定** —— D 见 §0 顶部订正横幅)、§5.5 全部真机数字 | 当前,逐节看横幅 |
...
| 你想知道 | 去 |
|---|---|
| **接手内核任务,从哪开始** | `agency/handoff-2026-08-10.md`(最新)→ `agency/handoff-2026-08-09.md` → `agency/handoff-2026-08-08.md` → `agency/handoff-2026-08-07.md`(§1 三条判据、§8.0 裁决)→ `agency/handoff-2026-08-06.md`(§3 真机七条、§7 诚实清单) |
| **搭桥(telly/god/ninja)怎么测、下一步做什么** | `agency/telly-test-plan.md` §1(几何/相位的拆分)§2(Phase 0,现在就能做)§8(顺序是硬的) |
| **那批变异候选跑完了吗** | 跑完了:**59/59 全 SURVIVED**,见 `agency/handoff-2026-08-10.md` §1,逐条锚点在 `mutation-candidates-2026-08-05.json` 的 `secondPassVerdict` |
...
| **下一轮做什么最划算** | `agency/handoff-2026-08-10.md` §6 —— **telly Phase 0**(不需要 owner 输入),或继续那 **58 条**待修 survivors(`Compat.java:124` 已修) |
