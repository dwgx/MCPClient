...
> hold 通道两次 7/7(`bfb12c3`)。**仍然是纯静态的**:§6 第 8 项(`create_tool` 那条路)、
> `do_click_slot` 的锁窗序列(只读过 vanilla 源码,没在真服上见它发生)、
> 以及导航的三条(四方向一次干净全过、**对角线至今没有任何一次到达**、1 格台阶)——
> 最后那三条现在有一条命令可以答:`scripts/live-nav-probe.py`,**但它自己还没跑过**。
>
...
> **进度(2026-08-04):1-4 与 6 已落地,7 一半落地,5 未做。** 原表保留,因为排序的理由
...
>   **但没有注册任何 MCP 工具** —— 它该要哪个 L4 权限是 owner 的决定。
> - **5** LOOK 的追踪不自终止模式:未做,而且它现在是这七步里唯一没动过的。
