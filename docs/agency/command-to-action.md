...
>
> **2026-08-06:导航那三条与 LOOK 追踪都验过了。** 三个探针 35 条全绿
> (LOOK 11/11、hold 13/13、nav 11/11),**而对角线第一次到达**(0.32 格,
> forward/strafe 各 0.707)。跑真机这一轮抓到四个缺陷,两个在生产侧,**一个是当轮作者
> 自己的误诊** —— 见 `handoff-2026-08-06.md` §3,它比这里的结论值得读。
>
> **仍然是纯静态的**:§6 第 8 项(`create_tool` 那条路)、`do_click_slot` 的锁窗序列
> (只读过 vanilla 源码,没在真服上见它发生)、以及 **craft 点击侧**(现在是唯一
> 没被真机碰过的通道)。
>
